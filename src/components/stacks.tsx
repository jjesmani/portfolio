...
import { Tool, getToolbelt } from "./toolbelt";
...
const Stacks: React.FC = () => {
	...
	return (
		<div>
			<h2 className="my-4 text-4xl font-bold">Curious of my stack?</h2>
			<div>
				<h3 className="text-3xl font-bold">Front End Stack</h3>
				<div className="flex flex-row flex-wrap gap-4 my-4">
					{frontendStack.map((stack) => (
						<ToolItem tool={stack} key={stack.id} />
					))}
				</div>
			</div>
			<div>
				<h3 className="text-3xl font-bold">Styling Stack</h3>
				<div className="flex flex-row flex-wrap gap-4 my-4">
					{stylingStack.map((stack) => (
						<ToolItem tool={stack} key={stack.id} />
					))}
				</div>
			</div>
			<div>
				<h3 className="text-3xl font-bold">Back End Stack</h3>
				<div className="flex flex-row flex-wrap gap-4 my-4">
			...
			<div>
				<h3 className="text-3xl font-bold">Other Stack</h3>
				<div className="flex flex-row flex-wrap gap-4 my-4">
					{otherStack.map((stack) => (
						<ToolItem tool={stack} key={stack.id} />
					))}
				</div>
			</div>
